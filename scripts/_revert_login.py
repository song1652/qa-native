"""Revert login retry logic back to original simple login."""
import re
...
for fpath in files:
    ...
    if OLD_WITH_RETRY in content:
        new_content = content.replace(OLD_WITH_RETRY, NEW_LOGIN, 1)
        fpath.write_text(new_content, encoding="utf-8")
        fixed += 1

print(f"Reverted: {fixed} files")
