...
scripts/            단계별 실행 스크립트 (LLM 없음, 순수 Python, 패키지 구조)
  ...
  04_approve.py     (사용 중단 — 승인 단계 제거, step=reviewed는 03_lint.py가 담당)
  05_execute.py     pytest 실행 (--only-failed, --no-report 플래그, 최대 8 workers)
  06_heal.py        실패 분석 (최대 3회 자동 패치, OMC ultraqa)
  06_auto_heal.py   자동 힐링 패치 (6개 정적 패턴 + heal_stats 빈출 패턴 보고)
...
agents/             사수-부사수 에이전트 시스템
  ...
  dashboard/
    ...
    serve.py        대시보드 로컬 서버 (python agents/dashboard/serve.py, 포트 8765)
    static/         정적 자산
...
tests/              테스트 산출물 + 리포트 + 스크린샷
  generated/        Claude Code가 작성한 테스트 코드 (그룹별 서브폴더, 각 __init__.py 필수)
    ...
    demoqa/         demoqa 그룹 (120개 테스트)
      __init__.py
      tc_*.py       개별 테스트 파일
    heroku/         heroku 그룹 (120개 테스트)
      __init__.py
      tc_*.py       개별 테스트 파일
