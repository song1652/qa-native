"""Playwright 테스트 — test_dragabble_axis_x (tc_88)"""
...
def test_dragabble_axis_x(page):
    """Drag restricted to X axis"""
    ...
    el = page.locator("#restrictedX")
    expect(el).to_be_visible(timeout=5000)
    el.scroll_into_view_if_needed()
    page.wait_for_timeout(500)
    _remove_ads(page)

    orig_style = el.evaluate("e => e.style.transform || e.style.left || ''")
    ...
    new_style = el.evaluate("e => e.style.transform || e.style.left || ''")
